...
static const char * TZAPP_PARTITION = "/dev/block/platform/13500000.dwmmc0/by-name/EFS";
...
#if !defined(PRODUCT_SHIP) || defined(USE_LDU) // not release build
    r = 0;
    goto exit;
#endif
...
int tz_tear_down(void){
    ...
    //for efs logging , we do not unmount efs partition
...
void dm_verity_recovery_end(void){
    ...
    //for efs logging , we do not unmount efs
    /*if(ensure_path_unmounted(EFS_MNT)){
    	printf("failed to umount %s\n", EFS_MNT);
    }*/
...
int dm_verity_verify(int mode) {
    ...
    if (!ret)
        ...
        do {
            ...
            ret = dm_verity_rehash();
            if(ret == DMVERITY_DRK_ERROR1  ||
               ret == DMVERITY_DRK_ERROR2  ||
               ret == NO_DRK_NEW_LIBDEVKM 
            ) drk_err = 1 ;
            if (ret < 0 && !drk_err) {
                printf("Rehashing failed. drk_err : %d \n" , drk_err);
                usleep(100*1000);
            }
            else {
                sync();
...
sign_failed:
    ...
    if (!sign_success && retry_cnt > 0 
                      && ret != DMVERITY_DRK_ERROR1 
                      && ret != DMVERITY_DRK_ERROR2
                      && ret != NO_DRK_NEW_LIBDEVKM
#ifdef SDM_660
                      && ret != DMVERITY_DRK_ERROR3
#endif
        ) {
...
#ifdef __USE_KAP
    if (ret == DMVERITY_DRK_ERROR1 || ret == DMVERITY_DRK_ERROR2 || ret == NO_DRK_NEW_LIBDEVKM
#ifdef SDM_660
        || ret == DMVERITY_DRK_ERROR3
#endif
        ) {
...
int dm_verity_rehash(void)
{
    ...
    usleep(100*1000);
#endif
    if(ret = execute_sign_blob(tmp_hash_table)) {
        //ret = -1;
        goto out;
    }
