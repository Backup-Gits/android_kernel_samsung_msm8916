...
int generate_signature(char *input, int input_size, char **result, uint32_t *result_size)
{
...
#ifdef USE_KEYSESSION_API
	char path[4096];
	ServiceKeyInfo_t keyInfo;
	memset(&keyInfo, 0, sizeof(ServiceKeyInfo_t)); 

	KeyType_t keyType = KEY_TYPE_RSA ;
	keyInfo.keyType = (uint8_t)keyType;
	...
	ret = createServiceKeySession(&keyInfo , buf , &buf_len , NULL , 0 ); 
	if (NO_ERROR != ret) {
		fprintf(stderr, "createSessionKey failed with returned code %d :(\n", ret);
		return -ret;
	} else {
		printf("createSessionKey success\n");
	}
