...
cd kernel-out/zip
zip -r -9 - * > ../"DRAGON-J510FN-$S_VERSION-$S_DATE.zip"
cd ../../
...
export CROSS_COMPILE=$(pwd)/dragon/gcc-linaro-4.9.4-2017.01-x86_64_arm-eabi/bin/arm-eabi-
mkdir -p output kernel-out/pack/rd kernel-out/zip/dragon/modules kernel-out/zip/dragon/kernel
make -C $(pwd) O=output msm8916_sec_defconfig VARIANT_DEFCONFIG=msm8916_j5lte_defconfig SELINUX_DEFCONFIG=selinux_defconfig
make -j`grep processor /proc/cpuinfo|wc -l` -C $(pwd) O=output
...
cp -r $(pwd)/output/drivers/staging/prima/wlan.ko $(pwd)/kernel-out/zip/dragon/modules/wlan
cd kernel-out/zip
zip -r -9 - * > ../"DRAGON-J500F-G-M-$S_VERSION-$S_DATE.zip"
cd ../../
...
export CROSS_COMPILE=$(pwd)/dragon/gcc-linaro-4.9.4-2017.01-x86_64_arm-eabi/bin/arm-eabi-
mkdir -p output kernel-out/pack/rd kernel-out/zip/dragon/modules kernel-out/zip/dragon/kernel
make -C $(pwd) O=output msm8916_sec_defconfig VARIANT_DEFCONFIG=msm8916_j5nlte_defconfig SELINUX_DEFCONFIG=selinux_defconfig
make -j`grep processor /proc/cpuinfo|wc -l` -C $(pwd) O=output
...
cp -r $(pwd)/output/drivers/staging/prima/wlan.ko $(pwd)/kernel-out/zip/dragon/modules/wlan
cd kernel-out/zip
zip -r -9 - * > ../"DRAGON-J500FN-$S_VERSION-$S_DATE.zip"
cd ../../
...
export CROSS_COMPILE=$(pwd)/dragon/gcc-linaro-4.9.4-2017.01-x86_64_arm-eabi/bin/arm-eabi-
mkdir -p output kernel-out/pack/rd kernel-out/zip/dragon/modules kernel-out/zip/dragon/kernel
make -C $(pwd) O=output msm8916_sec_defconfig VARIANT_DEFCONFIG=msm8916_j53g_defconfig SELINUX_DEFCONFIG=selinux_defconfig
make -j`grep processor /proc/cpuinfo|wc -l` -C $(pwd) O=output
...
cp -r $(pwd)/output/drivers/staging/prima/wlan.ko $(pwd)/kernel-out/zip/dragon/modules/wlan
cd kernel-out/zip
zip -r -9 - * > ../"DRAGON-J500H-$S_VERSION-$S_DATE.zip"
cd ../../
