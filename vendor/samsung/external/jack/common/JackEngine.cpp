...
int JackEngine::ClientStopJackd(int refnum, int param)
{
    if ((CLIENT_NUM <= refnum) || (0 > refnum)) {
         return -1;
    }
	...
	//TODO
    JackClientInterface* client = fClientTable[refnum];
    jack_log("JackEngine::ClientStopJackd ref = %ld name = %s", refnum, client->GetClientControl()->fName);

    return 0;
}


int JackEngine::ClientSwitchMaster(int refnum, int type, int param)
{
    if ((CLIENT_NUM <= refnum) || (0 > refnum)) {
         return -1;
    }
	...
	if( 1 == type || 2 == type ){  // alsa or opensles
...
#if defined (__ANDROID__) && defined (ENABLE_JACK_LOGGER)
int JackEngine::ClientStartDump(int refnum, int type, int param)
{
    if ((CLIENT_NUM <= refnum) || (0 > refnum)) {
         return -1;
    }
    ...
    JackClientInterface* client = fClientTable[refnum];
    jack_log("JackEngine::ClientStartDump ref = %ld name = %s", refnum, client->GetClientControl()->fName);
	mJackLogger->excuteCommand(JackLogger::LOG_COMMAND_PCM_DUMP, JackLogger::COMMAND_PARAM_PCM_DUMP_START);
    return 0;
}

int JackEngine::ClientStopDump(int refnum, int param)
{
    if ((CLIENT_NUM <= refnum) || (0 > refnum)) {
         return -1;
    }
...
int JackEngine::ClientDump(int refnum, int param)
{
    if ((CLIENT_NUM <= refnum) || (0 > refnum)) {
         return -1;
    }
    ...
    JackClientInterface* client = fClientTable[refnum];
    jack_log("JackEngine::ClientDump ref = %ld name = %s", refnum, client->GetClientControl()->fName);
	mJackLogger->dumpLog(0, NULL, JackLogger::LOG_BASE_JACKD);
    return 0;
}
#endif
